...
ggplot(btb.long, aes(x = occasion, y = score, color = treatment)) +
  ...
  labs(title = "BDI Score Trends Over Time", x = "Time (Occasion)", y = "BDI Score") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
```{r}
...
```{r}
summary_table %>%
  kable(format = "html", caption = "Number of Participants per Treatment Group", align = "c") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  add_header_above(c(" " = 2, "Treatment Group" = 3))  # Merged header over TAU & BtheB
...
summary2_wide <- btb_summary2 %>%
  pivot_wider(names_from = treatment, values_from = c(mean_bdi, sd_bdi, lower_ci, upper_ci)) %>%
  rename(
    "Occasion" = occasion,
    "Mean (SD) - TAU" = mean_bdi_TAU,
    "95% CI - TAU" = lower_ci_TAU,
    "Mean (SD) - BtheB" = mean_bdi_BtheB,
    "95% CI - BtheB" = lower_ci_BtheB
  ) %>%
  mutate(
    `Mean (SD) - TAU` = paste0(round(`Mean (SD) - TAU`, 2), " (", round(sd_bdi_TAU, 2), ")"),
    `95% CI - TAU` = paste0("[", round(`95% CI - TAU`, 2), ", ", round(upper_ci_TAU, 2), "]"),
    `Mean (SD) - BtheB` = paste0(round(`Mean (SD) - BtheB`, 2), " (", round(sd_bdi_BtheB, 2), ")"),
    `95% CI - BtheB` = paste0("[", round(`95% CI - BtheB`, 2), ", ", round(upper_ci_BtheB, 2), "]")
  ) %>%
  select(Occasion, `Mean (SD) - TAU`, `95% CI - TAU`, `Mean (SD) - BtheB`, `95% CI - BtheB`)  # Reorder columns

colnames(summary2_wide) <- c("Occasion", "Mean (SD)", "95% CI", "Mean (SD)", "95% CI")

summary2_wide %>%
  kable(format = "html", caption = "Mean BDI Scores Over Time by Treatment Group", align = "c") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
