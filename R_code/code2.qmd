...
#1)linear model - unstructured
btb.long$occasion <- as.numeric(as.factor(btb.long$occasion))

mean.mod1 <- gls(score ~ drug + length + occasion + treatment:occasion,  
                 corr = corSymm(form = ~ occasion | id), 
                 weights = varIdent(form = ~ 1 | occasion), 
                 na.action = na.omit, data = btb.long)
summary(mean.mod1)
```
...
mean.mod2 <- gls(score ~ occasionc + drug + length + treatment:occasionc + treatment:occasion2, 
                 corr=corSymm(form= ~ occasion | id), 
                 weights = varIdent(form = ~ 1 | occasion), 
                 na.action = na.omit, data = btb.long)
...
mean.mod3 <- gls(score ~ occasion + length + drug, 
                 corr=corSymm(form = ~ occasion | id), 
                 weights = varIdent(form = ~ 1 | occasion), 
                 na.action = na.omit, data = btb.long)
summary(mean.mod3)
...
spline.mod1 <- gls(score ~ drug + length + occasion + knot + treatment:occasion + treatment:knot,
                   corr=corSymm(form = ~ occasion | id), 
                   weights = varIdent(form = ~ 1 | occasion), 
                   na.action = na.omit, data = btb.long)
summary(spline.mod1)
...
mean.mod1.ml <- gls(score ~ drug + length + occasion + treatment:occasion,  
                    corr=corSymm(form= ~ occasion | id), 
                    weights = varIdent(form = ~ 1 | occasion), 
                    na.action = na.omit, method = "ML", data = btb.long)

mean.mod2.ml <- gls(score ~ occasionc + drug + length + treatment:occasionc + treatment:occasion2,
                    corr=corSymm(form= ~ occasion | id), 
                    weights = varIdent(form = ~ 1 | occasion), 
                    na.action = na.omit, method = "ML", data = btb.long)

mean.mod3.ml <- gls(score ~ occasion + length + drug, 
                    corr=corSymm(form= ~ occasion | id), 
                    weights = varIdent(form = ~ 1 | occasion), 
                    na.action = na.omit, method="ML", data = btb.long)

spline.mod1.ml <- gls(score ~ drug + length + occasion + knot + treatment:occasion + treatment:knot,  
                      corr=corSymm(form= ~ occasion | id), 
                      weights = varIdent(form = ~ 1 | occasion), 
                      na.action = na.omit, method = "ML", data = btb.long)
...
#anova(mean.mod2.ml, spline.mod1.ml)
#Error in nlme::anova.lme(object = mean.mod2.ml, spline.mod1.ml) : 
...
rim1 <- lme(score ~ occasion + drug + length, random = ~ 1 | id,
...
ggplot(btb.long, aes(x = occasion, y = score, group = id, color = factor(treatment))) +
  ...
  labs(
    ...
    y = "BDI Score",
    color = "Treatment Group",
    title = "Spaghetti Plot of Marginal Mean Response Over Time"
  ) +
  theme_minimal()
...
mod1.slope <- lme(score ~ post + drug + length + post:treatment, random = ~ post | id, data = btb.long, na.action = na.omit,
...
mod1.slope.ml <- lme(score ~ post + drug + length + post:treatment, random = ~ post | id, 
                  method = "ML", data = btb.long, na.action = na.omit, control = lmeControl(opt = "optim", maxIter = 100, msMaxIter = 100))
anova(mod1.un.ml, mod1.slope.ml)

#since p-value is 0.2046 we the random slope does not add significant value to the model so we can stick with the more parsimonious model
```
...
mod1.slope.knot <- lme(score ~ post + drug + length + knot + post:treatment + knot:treatment, random = ~ post + knot| id, data = btb.long, na.action = na.omit,
                  control = lmeControl(opt = "optim", maxIter = 100, msMaxIter = 100))

summary(mod1.slope.knot)

mod1.slope.knot.ml <- lme(score ~ post + drug + length + knot + post:treatment + knot:treatment, random = ~ post + knot| id, data = btb.long, na.action = na.omit,control = lmeControl(opt = "optim", maxIter = 100, msMaxIter = 100), method = "ML")

anova(mod1.slope.knot.ml, mod1.un.ml)

#reject the null and conclude adding the spline significantly impacts the accuracy of the model so we use the spline model with a random intercept
```
